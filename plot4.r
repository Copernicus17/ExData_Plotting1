...
png(filename = "C:/Users/Larry/Documents/Coursera/ExData_Plotting1/plot4.png",
                width = 480, height = 480)

  par(mfrow = c(2,2))  ## 2x2 panels filled by rows
  par(oma = c(0,0,1,0))  ## default outer margin is no margin i.e. (0,0,0,0)
  
  ## Plot 4.1.1: Global_active_power (in Kilowatts) vs. Datetime
  ## Global_active_power: household global minute-avg active power (in kilowatt)
  ## ---------------------------------------------------------------------------
  plot(tpower2$dtTime, tpower2$Global_active_power, type = "l", 
       xlab = "", ylab = "Global Active Power")
  title(main = "Plot 4", outer = TRUE, adj = 0)  #add left aligned title
  box()  ## draw box around plot


  ## Plot 4.1.2: Voltage vs. Datetime
  ## Global_active_power: Voltage: minute-averaged voltage (in volt)
  ##----------------------------------------------------------------------------
  plot(tpower2$dtTime, tpower2$Voltage, type = "l",
       xlab = "datetime", ylab = "Voltage")

  ## Plot 4.2.1: Energy sub-metering (in watt-hour of active energy) vs. Datetime 
  ## ----------------------------------------------------------------------------
  ...
  ## Plot 4.2.2: Bar plot of Sub_Metering (in Kilowatts)
  ## Global_reactive_power: household global minute-avg reactive power (in kilowatt)
  with(tpower2, barplot(Global_reactive_power, ylab = "Global_reactive_power",
                        xlab = "datetime",
                        ## names.arg = weekdays(dtTime, abbreviate = TRUE)
                        ))
  ## axis(1, labels = TRUE)
  box()  ## draw box around plot
